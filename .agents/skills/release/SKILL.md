...
name: release
description: >-
  执行项目发版与版本发布全流程（包括运行全量测试、本地编译验证、同步远程 tag、评估是否同步更新 README.md、生成并更新 CHANGELOG.md、评估语义化版本号 major/minor/patch、创建附注 tag 并推送至远端）。当用户提出“发版”、“发布新版本”、“打 tag 发版”、“做一次 release”或类似需求时使用该 skill。
---
...
## 阶段 5：更新发布文档（CHANGELOG.md 与 README.md）并提交
...
### 5.2 评估并同步更新 `README.md`

`README.md` 是面向新用户与开发者的**产品全貌规格说明**，而非时间线流水账。发版时应根据改动性质审慎评估是否更新，避免引入无意义的信息冗余。

#### 1. 什么时候该更新 README（触发判定）
| 变更场景 | 是否更新 README | 判定准则与说明 |
| :--- | :---: | :--- |
| **新增面向用户的重要核心能力** | **是** | 如新增支持新归档格式（Paged110/90CN 变体）、新增独立功能面板（动画预览/装备面板/脚本工作区）、新增关键导出/导入模式等宏观功能。 |
| **系统架构或关键模块增删改** | **是** | 如新增了纯 Go 核心模块（`internal/preview`）、新增服务层 Service、新增前台关键视图组件或核心架构设计文档。 |
| **构建、测试或前置环境变动** | **是** | 如调整了测试运行参数、新增了关键回归测试说明或更新了全局构建依赖。 |
| **项目元数据或全局规范调整** | **是** | 如开源协议变更（GPL 等）、全局快捷键增删、文件关联扩展名变动等。 |
| **纯 Bug 修复与异常分支防御** | **否** | 如修复下标越界、goroutine 泄漏、临时比对错误等，**仅记入 CHANGELOG**，切忌写入 README。 |
| **细微 UI 样式微调与局部文案优化**| **否** | 如按钮边距微调、弹窗 loading 状态反馈、右键菜单项文案润色等，**仅记入 CHANGELOG**。 |
| **常规日常重构与非核心单测补充**| **否** | 内部代码重命名、测试用例微调等不影响宏观架构的调整，保持 README 稳定。 |
| **纯补丁小版本（Patch，如 `v0.x.1`）**| **通常否** | 除非该补丁修正了 README 现存的错误说明或协议变更，通常保持 README 不动。 |

#### 2. 应该更新哪些内容（精准定位）
更新时应严格对齐章节职责，进行**局部精准更新**，严禁大面积推倒重写：
- **`📖 项目简介`**：仅当引入全局性的重大支持（如新增核心格式、里程碑级特性）时，在摘要 bullet points 中追加或精炼更新 1~2 句话。
- **`✨ 核心特性`**：
  - **新功能品类**：增加对应的 `###` 三级小节，提炼核心价值与关键技术指标。
  - **现有功能增强**：在现有对应小节中仅追加 1~2 条精简要点（例如：在脚本工作区小节追加列表与文件集 API 说明）。
- **`⌨️ 快捷键操作`**：仅当新增或调整全局高频快捷键时，更新快捷键表格。
- **`🏗️ 系统架构`**：若新增了关键前端组件、服务层 Service 或内部内核模块，在 ASCII 架构图及说明中同步补齐。
- **`🛠️ 开发与构建`**：若运行测试或打包验证有新增覆盖范围或环境变量，在对应命令段落中同步。
- **`📁 项目结构`**：若新增了长期维护的核心模块目录（`internal/*`）、关键服务文件、UI 核心组件或技术文档，同步更新目录树与简要说明。

#### 3. 如何避免过多的冗余（撰写规范）
- **职责分明，拒绝流水账**：
  - `CHANGELOG.md` 记录**时间线上发生了什么变化**（“本次版本修复了...”、“新版本重构了...”）；
  - `README.md` 说明**当前软件具备什么能力**（使用陈述句：“全面兼容 110US 分页归档与密钥链解密”，切忌使用“本次更新修复/新增了”等时效性过渡词）。
- **高密度提炼，加粗关键要点**：
  - 格式遵循：`- **核心概念/功能名**：简明扼要的技术能力阐述，说明支持范围与核心机制。`
  - 避免把内部函数名、临时排查过程、琐碎 edge case 写进 README。
- **格式一致性与可点击链接**：
  - 架构图、快捷键表格与目录树保持已有的排版与对齐风格，文档引用路径保持有效。

### 5.3 提交发布文档变更
将 `CHANGELOG.md`（以及若有更新的 `README.md` 和相关配置文件）一并提交：
```bash
git add CHANGELOG.md
# 若本次发版同步更新了 README.md 及其他元数据，一并暂存：
# git add README.md build/linux/nfpm/nfpm.yaml ...
