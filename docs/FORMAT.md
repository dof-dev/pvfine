...
但分段种子不再由 `"HeaD"/"HASH"/…` 这组固定字符串推出,因此标准密钥解不开头部。
`internal/pvf/variant.go` 对这类文件逐个反解种子:

- 头部:签名明文已知(固定为 `nkpi`),可限定首个密钥流 dword 的低 16 位,
  余下 16 位只有 65536 种;再用各段长度之和必须等于文件大小筛选出唯一解。
- GRPI:最后一块的累计压缩大小必须等于 `BodySize`,同样只剩 65536 种种子;
  再要求累计大小严格递增、各块原始大小为正,即可确定。
- Body 与 sTrA/sTrW:整段是 zlib 流,头部两字节 `78 xx` 是已知明文,
  可解出种子;再用 zlib 解压结果必须正好等于 GRPI/NameTable 声明的长度来确认。

恢复过程只在标准密钥失败时触发,单次打开约 2~4 秒。
