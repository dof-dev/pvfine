...
**此前未解决**：那个 uint32 的生成函数。要对上新条目，需要下面任一条件：
...
**已解决（2026-09-19）**：用 `testdata/110US.pvf` 的主列表在用条目以及
其它普通索引条目拟合并逐项校验后，新增 ID 的生成函数确认为无初始常量的
32 位可逆混合（两轮乘法）：

```text
x = ((x ^ (x >> 16)) * 0x45d9f3b)  // uint32 溢出保留低 32 位
x = ((x ^ (x >> 16)) * 0x45d9f3b)
value = x ^ (x >> 16)
```

因此 `id=0` 的值为 0；例如 `id=1` → `824515495`，`id=1000` →
...
回归测试覆盖 `equipment/stackable/monster/npc/appendage` 的在用条目。
